...
var last_status = {};
global.setCmdAlias = {};
global.hanstztechOfficial = false;
global.sqldb = false;
...
async function syncdb() {
  let thumbnailImagePath = __dirname + "/assets/hanstztech.jpeg";
  ...
  try {
    ...
    thumbnailImagePath = __dirname + "/assets/hanstztech.jpeg";
  }
  global.log0 =
  ...
  let hanstztechConn = hanstztechConnectSock({
    logger: Pino({ level: "silent" || "debug" || "fatal" }),
    printQRInTerminal: false,
    browser: ["Windows", "chrome", "MATRIX-XMD"],
  ...
  hanstztechConn.ev.on("call", async (callData) => {
    ...
    events.commands.map(async (command) => {
      if (command.call === "offer" && callResponse.status === "offer") {
        try {
          command.function(callResponse, { store: store, Void: hanstztechConn });
        } catch (error) {
  ...
  let dbgroup = {};
  let dbuser = {};
  hanstztechConn.ev.on("messages.upsert", async (callData) => {
    try {
      if (!callData.messages || !Array.isArray(callData.messages)) {
        return;
      }
      botNumber = botNumber || hanstztechConn.decodeJid(hanstztechConn.user.id);
  ...
  hanstztechConn.ev.on("messages.upsert", async (callData) => {
    try {
      ...
      for (mek of callData.messages) {
        ...
        if (blockJid.includes(messageData.chat) && !messageData.ishanstztech) {
          return;
        }
        if (
          isCommand &&
        ...
        if (isCommand) {
          ...
          if (messageData.isGroup && command && commandName !== "bot") {
            let groupData = dbgroup[messageData.chat] ||
              (await groupdb.findOne({
                ...
                botenable: toBool(
                  messageData.ishanstztech || !blockJid.includes(messageData.chat)
                ),
              };
          ...
          if (command) {
            ...
            if (category) {
              ...
              hanstztechConn.sendUi(messageData.jid, {
        ...
        events.commands.map(async (command) => {
          if (typeof command.on === "string") {
            ...
            if (commandName === "main" && shouldExecute) {
              ...
              messageData.text &&
              commandName === "text" &&
              /text|txt|true|smd|hanstztech/gi.test(command.quoted) &&
              messageData.quoted &&
      ...
      try {
        ...
        events.commands.map(async (command) => {
          if (groupData.status === command.group) {
            try {
              command.function(groupData, { store: store, Void: hanstztechConn });
            } catch (error) {
              console.error("[GROUP PARTICIPANTS ADD ERROR] ", error);
            }
          }
          if (/on|true|main|all|hanstztech|smd/gi.test(command.group)) {
            try {
              command.function(groupData, { store: store, Void: hanstztechConn });
            } catch (error) {
  ...
  hanstztechConn.ev.on("groups.update", async (groupsUpdate) => {
    try {
  ...
  hanstztechConn.ev.on("groups.upsert", async (groupsUpsert) => {
    try {
      events.commands.map(async (command) => {
        if (
          /on|true|main|all|hanstztech|smd/gi.test(
            command.groupsetting || command.upsertgroup || command.groupupsert
          )
        ) {
          command.function(
            { ...groupsUpsert[0], bot: hanstztechConn },
            { store: store, Void: hanstztechConn, data: groupsUpsert }
          );
        }
      });
      await groupsg(
  ...
  hanstztechConn.ev.on("contacts.upsert", (contactsUpsert) => {
    try {
  ...
  hanstztechConn.ev.on("contacts.update", async (contactsUpdate) => {
    for (let contact of contactsUpdate) {
      let decodedJid = hanstztechConn.decodeJid(contact.id);
      if (store && store.contacts) {
        store.contacts[decodedJid] = { id: decodedJid, name: contact.notify };
      }
    }
  });
  hanstztechConn.serializeM = (message) => smsg(hanstztechConn, message, store, false);
  hanstztechConn.ev.on("connection.update", async (_0x3fd0b3) => {
    const {
    ...
    if (_0x2224a7 === "open") {
      if (
        ...
        !hanstztechConn.authState.creds?.myAppStateKeyId
      ) {
        log(
        ...
        hanstztechConn.ev.session_reset();
      }
      let _0x89b8f6 = hanstztechConn.decodeJid(hanstztechConn.user.id);
            ...
            try {
              ...
              if (_0x28bc47) {
                let _0x224d22 =
                  _0x5701b6 +
                  (_0x4fcba7[_0x5701b6] &&
                  /.js|.smd|.hanstztech/gi.test(_0x4fcba7[_0x5701b6])
                    ? _0x4fcba7[_0x5701b6]
                    : ".smd");
                const _0x286e03 =
                  plugin_dir +
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "MATRIX-XMD") +
        "\n";
      try {
      ...
      print(_0x1f88ec);
      await hanstztechConn.sendMessage(
        _0x89b8f6,
        {
          text: "```" + ("" + _0x1f88ec).trim() + "```",
        },
  ...
  hanstztechConn.ev.on("creds.update", creds);
  hanstztechConn.lastStatus = async () => {
    console.log("last_status :", last_status);
    return last_status;
  };
  // Decodes a JID (Jabber ID) string
  hanstztechConn.decodeJid = (jid) => {
    if (!jid) {
      return jid;
    }
    if (/:\d+@/gi.test(jid)) {
  ...
  hanstztechConn.getName = (jid, withoutContact = false) => {
    let decodedJid = hanstztechConn.decodeJid(jid);
    let contact;
    ...
    if (decodedJid.endsWith("@g.us")) {
      return new Promise(async (resolve) => {
        ...
        if (!contact.name?.notify && !contact.subject) {
          try {
            contact = (await hanstztechConn.groupMetadata(decodedJid)) || {};
          } catch (error) {}
        }
      ...
      contact =
        decodedJid === "0@s.whatsapp.net"
          ...
          : decodedJid === hanstztechConn.decodeJid(hanstztechConn.user.id)
          ? hanstztechConn.user
  ...
  hanstztechConn.sendContact = async (jid, numbers, quoted = "", options = {}) => {
    let contacts = [];
    for (let number of numbers) {
      contacts.push({
        displayName: await hanstztechConn.getName(number + "@s.whatsapp.net"),
        vcard:
          "BEGIN:VCARD\nVERSION:3.0\nN:" +
          (await hanstztechConn.getName(number + "@s.whatsapp.net")) +
          "\nFN:" +
          global.OwnerName +
          "\nitem1.TEL;waid=" +
          number +
    ...
    return hanstztechConn.sendMessage(
      jid,
      {
        contacts: {
  ...
  hanstztechConn.setStatus = (status) => {
    hanstztechConn.query({
      tag: "iq",
      attrs: {
        to: "@s.whatsapp.net",
        type: "set",
  ...
  hanstztechConn.messageId = (length = 8, prefix = "MATRIX-XMD") => {
  ...
  hanstztechConn.send5ButImg = async (
    jid,
    caption = "",
    footer = "",
    image,
    ...
    let mediaInfo = await prepareWAMessageMedia(
      ...
      {
        upload: hanstztechConn.waUploadToServer,
      }
    );
    ...
    hanstztechConn.relayMessage(jid, message.message, {
      messageId: hanstztechConn.messageId(),
    });
  };

  // Sends a button message with text
  hanstztechConn.sendButtonText = (
    jid,
    buttons = [],
    text,
    footer,
    ...
    hanstztechConn.sendMessage(jid, buttonMessage, {
      quoted: quoted,
      ...options,
    });
  };
  ...
  hanstztechConn.sendText = (jid, text, quoted = "", options) =>
    hanstztechConn.sendMessage(
      jid,
      {
        text: text,
        ...options,
  ...
  hanstztechConn.sendImage = async (
    jid,
    image,
    caption = "",
    quoted = "",
    ...
    let buffer = Buffer.isBuffer(image)
      ...
      : Buffer.alloc(0);
    return await hanstztechConn.sendMessage(
      jid,
      {
        image: buffer,
        caption: caption,
  ...
  hanstztechConn.sendTextWithMentions = async (jid, text, quoted, options = {}) =>
    hanstztechConn.sendMessage(
      jid,
      {
        text: text,
  ...
  hanstztechConn.sendImageAsSticker = async (jid, image, options = {}) => {
    let webpImage;
  ...
  hanstztechConn.sendVideoAsSticker = async (jid, video, options = {}) => {
    let webpVideo;
    if (options && (options.packname || options.author)) {
  ...
  hanstztechConn.sendMedia = async (
    jid,
    media,
    fileName = "",
    caption = "",
    ...
    let fileData = await hanstztechConn.getFile(media, true);
  ...
  hanstztechConn.downloadAndSaveMediaMessage = async (
    message,
    fileName = "null",
    returnBuffer = false,
    createFile = true
    ...
    try {
      ...
      if (type == "conversation") {
        ...
        for (let jid of parsedJid(jids)) {
          await hanstztechConn.sendMessage(jid, forwardOptions, {
            quoted: quoted,
            messageId: hanstztechConn.messageId(),
          });
        }
  ...
  hanstztechConn.downloadMediaMessage = async (message) => {
    let msg = message.msg ? message.msg : message;
    let mimeType = (message.msg || message).mimetype || "";
    let mediaType = message.mtype
      ? message.mtype.replace(/Message/gi, "")
  ...
  hanstztechConn.forwardOrBroadCast2 = async (
    jid,
    message,
    options = {},
    type = ""
    ...
    try {
      ...
      await hanstztechConn.relayMessage(jid, forwardedMessage.message, {
        messageId: hanstztechConn.messageId(),
      });
  ...
  hanstztechConn.forwardOrBroadCast2 = async (
    ...
    try {
      ...
      await hanstztechConn.relayMessage(_0x2565bd, _0x5d8b14.message, {
        messageId: hanstztechConn.messageId(),
      });
      return _0x5d8b14;
    } catch {}
  };
  hanstztechConn.forwardOrBroadCast = async (
    _0x189e5c,
    _0x54c0d9,
    _0x18e0cf = {},
    _0x348d78 = ""
    ...
    try {
      ...
      _0x18e0cf.messageId = _0x18e0cf.messageId || hanstztechConn.messageId();
      ...
      await hanstztechConn.relayMessage(_0x189e5c, _0xf31bf3.message, {
  ...
  hanstztechConn.forwardMessage = hanstztechConn.forwardOrBroadCast;
  hanstztechConn.copyNForward = async (
    _0x333b21,
    _0x4d04c0,
    _0x3c23fa = false,
    _0x4ed2c0 = {}
    ...
    try {
      ...
      await hanstztechConn.relayMessage(_0x333b21, _0x3f7fe3.message, {
        messageId: hanstztechConn.messageId(),
      });
      return _0x3f7fe3;
    } catch (_0x529a5c) {
  ...
  hanstztechConn.sendFileUrl = async (
    _0x245d2a,
    _0x1ddbcd,
    _0xd689ee = "",
    _0x2cf1f3 = "",
    ...
    try {
      ...
      if (_0x5a6000) {
        try {
          return await hanstztechConn.sendMessage(_0x245d2a, _0x5a6000, {
            quoted: _0x2cf1f3,
          });
        } catch {}
      }
  ...
  hanstztechConn.sendFromUrl = hanstztechConn.sendFileUrl;
  const _0x29f5c5 = {};
  let _0x51034c = [];
  hanstztechConn.sendUi = async (
    _0x264148,
    _0x4d42ab = {},
    _0x541cb3 = "",
    _0x2e5e1f = "",
    ...
    try {
      ...
      if (
        (((_0x171bcf && _0x3bceba && global.style > 0) || !_0x3bceba) &&
          /text|txt|nothing|smd|hanstztech/.test(global.userImages)) ||
        _0x2e5e1f == "text"
      ) {
        _0x237b02 = {
      ...
      const _0x28e991 =
        _0x171bcf && _0x3bceba && global.style > 0
          ...
          : null;
      _0x28cdb7 = {
        ...(await hanstztechConn.contextInfo(
          Config.botname,
    ...
    try {
      return await hanstztechConn.sendMessage(_0x264148, {
        image: {
          url: await botpic(),
        },
        contextInfo: _0x28cdb7,
      ...
      return hanstztechConn.sendMessage(_0x264148, {
        text: _0x4d42ab.text || _0x4d42ab.caption,
        ..._0x4d42ab,
      });
    }
  };
  hanstztechConn.contextInfo = async (
  ...
  hanstztechConn.cMod = (
    ...
    _0x4510a5,
    _0xaaa44a = "",
    _0x2f32eb = hanstztechConn.user.id,
    _0x18c25b = {}
  ) => {
    ...
    _0x4510a5.key.fromMe = _0x2f32eb === hanstztechConn.user.id;
    return proto.WebMessageInfo.fromObject(_0x4510a5);
  };
  hanstztechConn.getFile = async (_0x45942b, _0x80d77a) => {
    let _0x5bc7b0;
  ...
  hanstztechConn.sendFile = async (
    ...
    let _0x4285a2 = await hanstztechConn.getFile(_0x2edb21, true);
    let {
      filename: _0x479138,
      size: _0x527df9,
      ext: _0x14fc23,
  ...
  hanstztechConn.fakeMessage = async (
    _0x141089 = "text",
    _0x5364cd = {},
    _0x371309 = "➬ hanstztech SER",
    _0x1a0d91 = {}
  ) => {
    const _0x22700d = [777, 0, 100, 500, 1000, 999, 2021];
    let _0x17661c = {
      id: hanstztechConn.messageId(),
      fromMe: false,
  ...
  hanstztechConn.parseMention = async (_0x3d4032) => {
  ...
  app.get("/chat", (_0x52c0af, _0x203368) => {
    let _0x5785a4 =
      ...
      _0x52c0af.query.jid ||
      hanstztechConn.user.id ||
      hanstztechConn.user.m ||
      "";
    ...
    _0x5785a4 = hanstztechConn.decodeJid(_0x5785a4);
    const _0x382b66 =
      (
  ...
  hanstztechConn.dl_size = global.dl_size || 200;
  hanstztechConn.awaitForMessage = async (_0x3f601c = {}) => {
    return new Promise((_0x42ef72, _0x15fbb5) => {
      ...
      let _0x4fedac = (_0x54d8e0) => {
        ...
        if (_0x4bad66 == "notify") {
          for (let _0xa0c0c6 of _0x5bf6f0) {
            ...
            const _0x5447cd = hanstztechConn.decodeJid(
              _0x3f5f64
                ? hanstztechConn.user.id
                : _0x36ea4b || _0x27529b
            ...
            if (
              ...
              hanstztechConn.ev.off("messages.upsert", _0x4fedac);
      ...
      hanstztechConn.ev.on("messages.upsert", _0x4fedac);
      if (_0x358393) {
...
global.toBool = (value, defaultValue = false) =>
  /true|yes|ok|act|sure|enable|smd|hanstztech/gi.test(value)
    ? defaultValue
      ? true
      : "true"
    : defaultValue
  ...
  try {
    fs.readdirSync(pluginsDir).forEach((file) => {
      ...
      if (fs.statSync(filePath).isDirectory()) {
        ...
        [".js", ".smd", ".hanstztech"].includes(path.extname(file).toLowerCase())
      ) {
        try {
          require(filePath);
        } catch (error) {
...
app.get("/hanstztech", (request, response) => response.type("html").send(html));
...
app.listen(randomPort, () =>
  console.log(
    "MATRIX-XMD Server listening on http://localhost:" + randomPort + "/ "
  )
);
...
async function MakeSession(
  ...
  let decodedSessionId = ("" + sessionId)
    ...
    .replace(/^hanstztech_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "")
    .replace(/Secktor;;;/gi, "")
    .replace(/Astro;;;/gi, "")
    .replace(/hanstztech;;;/gi, "")
    .trim();
