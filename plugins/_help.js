...
╚════════════════════╝
      \n *MATRIX-XMD SIMPLE WHATSAPP BOT*\n
╔════❰ *ALL MENU* ❱ 
║🎗️║ Lɪꜱᴛ 
║🎗️║ Cᴀᴛᴇɢᴏʀʏ 
║🎗️║ Hᴇʟᴘ 
...
await message.bot.sendMessage(
        message.chat,
        {
          audio: { url: "https://files.catbox.moe/perl56.jpeg" },
          mimetype: "audio/mpeg",
        },
        { quoted: message }
      );

      // Send the menu message with the curved box design
...
astro_patch.smd(
  ...
  async (message) => {
    ...
    const { key } = await message.reply("*Testing Ping‼️*");
    var endTime = new Date().getTime();

    // Define the MP3 file and MIME type
    const mp3Url = "https://www.example.com/ping-sound.mp3"; // Replace with your actual MP3 URL
    const mimeType = "audio/mpeg"; // Standard MIME type for MP3 files

    // Download and attach the MP3 file (if sending directly as media)
    const response = await fetch(mp3Url);
    const mp3Buffer = await response.buffer();

    // Send the response with the MP3 file and calculated ping
    return await message.send(
      {
        caption: `*MATRIX-XMD SPEED*\n *${endTime - startTime} ms*`,
        file: mp3Buffer,
        mimetype: mimeType,
        filename: "ping-sound.mp3",
      },
      {
        edit: key,
      },
