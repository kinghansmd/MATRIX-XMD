...
smd({
    ...
    async({reply, chat ,isGroup, sender,error}) => {
      try{
        ...
        const daily  = await eco.daily(sender, "MATRIX-XMD", 500); //give 500 for daily, can be changed
        if (daily.cd) {
...
smd({
    ...
    async(message) => {
      try{
       ...
       const balance  = await eco.balance(users, "MATRIX-XMD")
       await eco.deduct(users, "MATRIX-XMD", balance.wallet);
...
async(message,match) => {
  try{
   ...
   const balance  = await eco.balance(user, "MATRIX-XMD")
   switch (value) {
       case '1000':
       case '1':
       if (k > balance.wallet ) return message.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`);
         const deduct1 = await eco.deduct(user, "MATRIX-XMD", 100);
         const add1 = eco.giveCapacity(user,"MATRIX-XMD", 1000);
       ...
       if (k < balance.wallet) return message.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`);
         const deduct2 = await eco.deduct(user,"MATRIX-XMD", 1000);
         const add2 = eco.giveCapacity(user, "MATRIX-XMD", 100000);
       ...
       if (k < balance.wallet) return message.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`);
          const deduct3 = await eco.deduct(user, "MATRIX-XMD", 10000);
          const add3 = eco.giveCapacity(user, "MATRIX-XMD", 10000000);
...
await message.reply('*What are you trying to do📉*.')
   ...
   async(message,match) => {
     ...
     //  let users = message.mentionedJid ? message.mentionedJid[0] : message.msg.contextInfo.participant || false;
       ...
       const deposit = await eco.deposit(message.sender, "hanstztech", d);
       const balance = await eco.balance(message.sender, "hanstztech")
   ...
   async(message) => {
     try{
   let h = await eco.lb("hanstztech",10);
...
async(message,match) => {
  try{
   ...
   if(!user) return message.reply('Please give me any user🤦‍♂️.');
       ...
       const balance = await eco.balance(user1, "hanstztech");
       let a = (balance.wallet) < parseInt(word)
       //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
       if(a == true) return message.reply("you dont have sufficient money to transfer👎");

       const deduct = await eco.deduct(user1, "hanstztech", value[0]);
       const give = await eco.give(user2, "hanstztech", value[0]);
  ...
  // return await Aviator.bot.sendButtonText(message.chat, `*📠 Transaction successful of ${value[0]} 💰*`, `${Config.ownername.split(' ')[0]}-Economy Version: 0.0.6`, message);
   ...
   async(message) => {
     try{
       ...
       let mongoschemas = zerogroup.economy || "false";
       if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
        const balance = await eco.balance(message.sender,"hanstztech"); //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
   ...
   async(message,match) => {
     try{
       if(!message.isCreator) return message.reply(`*_Hey Master, only my owner can give money!_*`)
        ...
        await eco.give(users, "hanstztech", parseInt(match.split(' ')[0]));
   ...
   if(!users) return message.reply('Please give me user to rob.')
       ...
       const user2 = users
       const k = 1000
       const balance1  = await eco.balance(user1, "hanstztech")
   const balance2  = await eco.balance(user2, "hanstztech")
   ...
   switch (random) {
     ...
     const deduff = Math.floor(Math.random() * 1000)	    
         await eco.deduct(user2, "hanstztech", deduff);
         await eco.give(message.sender, "hanstztech", deduff);
       ...
       case 'caught':
          ...
          await eco.deduct(user1, "hanstztech", rmoney);
...
///.mentionedJid[0] ? m.mentionedJid[0] : m.sender
    const balance = await eco.balance(user, "hanstztech");
  ...
  message.reply(media,{packname:"hanstztech",author:'Economy'},"sticker")
    ...
    if (!match) return message.reply(`Example:  ${prefix}gamble 100 direction(left,right,up,down)`);
          ...
          if ( r == opp){
          let give = await eco.give(user , "hanstztech", twice);    //message.react('⭐️')
   ...
   //return await Aviator.bot.sendButtonText(message.chat, `*📈 You won 🪙${twice}*`, `${Config.ownername.split(' ')[0]}-Economy \n Version: 0.0.6`, message);
                 ...
                 let deduct = await eco.deduct(user, "hanstztech", texts[0]);
   ...
   async(message,match) => {
     try{
       ...
       if (today.getDay() == 6 || today.getDay() == 5 || today.getDay() == 0){
           ...
           const user = message.sender
           const k = 100
           const balance1  = await eco.balance(user,"hanstztech")
...
if(value<=balance.wallet){
   ...
   if ((f1 !== f2) && f2 !== f3){
       const deduct1 = await eco.deduct(user, "hanstztech", deduff);
       return message.reply(`${mess1}\n\n*Big Lose -->* _🪙${deduff}_`)
    }
    else if ((f1 == f2) && f2 == f3){
       const give1 = await eco.give(user, "hanstztech", deduff/2);
       return message.reply(`${mess2}\n*_Little Jackpot -->* _🪙${deduff/2}_`)
    }
    else if ((f1 == f2) && f2 !== f3){
       const give2 = await eco.give(user, "hanstztech", deduff);
       return message.reply(`${mess3}\n*Small Win -->* _🪙${deduff}_`)
    }
    else if ((f1 !== f2) && f1 == f3){
       const deduct2 = await eco.deduct(user, "hanstztech", deduff);
       return message.reply(`${mess5}\n\n*Small Lose -->* _🪙${deduff}_`)
    }
    else if ((f1 !== f2) && f2 == f3){
       const give4 = eco.give(user,"hanstztech", deduff);
       return message.reply(`${mess3}\n\n*Small Win -->* _🪙${deduff}_`)
    }
    else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
       const give5 = eco.give(user,"hanstztech", deduff*20);
   ...
   return message.reply('You don\'t have enough 💰amount in your👛 wallet.\n- Please don\'t provide 🤑amount.')
           ...
           if ((f1 !== f2) && f2 !== f3){
              const deduct1 = await eco.deduct(user, "hanstztech", 50);
                     message.reply(`${mess1}\n\n*Big Lose -->* _🪙50_`)
           }
           else if ((f1 == f2) && f2 == f3){
              const give1 = await eco.give(user,"hanstztech", 100);
                    message.reply(`${mess2}\n*_Little Jackpot -->* _🪙100_`)
           }
           else if ((f1 == f2) && f2 !== f3){
              const give2 = await eco.give(user, "hanstztech", 20);
                    message.reply(`${mess3}\n*Small Win -->* _🪙20_`)
           }
           else if ((f1 !== f2) && f1 == f3){
              const deduct2 = await eco.deduct(user, "hanstztech", 20);
                    message.reply(`${mess5}\n\n*Small Lose -->* _🪙20_`)
           }
           else if ((f1 !== f2) && f2 == f3){
              const give4 = eco.give(user, "hanstztech", 20);
                    message.reply(`${mess3}\n\n*Small Win -->* _🪙20_`)
           }
           else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
              const give5 = eco.give(user, "hanstztech", 1000);
                   message.reply(`${mess4}\n\n_🎊 JackPot --> _🪙1000_`)
           }
...
async(message) => {
  try{
   ...
   if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
   const kg = 100
           const balance1  = await eco.balance(message.sender, "hanstztech")
   ...
   if(lol){
       ...
       const give2 = await eco.give(message.sender, "hanstztech", deduff*2);
  ...
  // return await Aviator.bot.sendButtonText(message.chat,str+`You got ${deduff*10} in your wallet.`, `${Config.ownername.split(' ')[0]}-Economy \n Version: 0.0.6`, message);
   ...
   const deduct1 = await eco.deduct(message.sender, "hanstztech", deduff);
   let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `
           let str = st.replace(/1/g, `🔴`).replace(/2/g, `🔵`).replace(/3/g, `🟣`).replace(/4/g, `🟢`).replace(/5/g, `🟡`).replace(/6/g, `⚪️`).replace(/7/g, `⚫️`).replace(/:/g, `    `)
